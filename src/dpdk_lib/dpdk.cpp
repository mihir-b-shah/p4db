...
struct SystemUtils {
    // TODO: are hugepages available / beneficial?
...
bool Dpdk::init(CPUSet cpus, size_t mbuf_pool_size, uint32_t masterCore) {
    ...
    // We need the uio_pci_generic driver?
    ...
    if (ret < 0) {
        LOG_ERROR("failed to init the Dpdk EAL, err=%s", rte_strerror(rte_errno));
        return false;
    }
